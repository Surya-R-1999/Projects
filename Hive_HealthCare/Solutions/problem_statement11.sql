...
DELIMITER $$
CREATE PROCEDURE IF NOT EXISTS contact_pharmacy(VAR1 INT)
BEGIN 
select ph.pharmacyID, ph.phone from pharmacy ph join keep k on ph.`pharmacyID` = k.`pharmacyID`
                          join Medicine m on m.`medicineID` = k.`medicineID`
where m.`medicineID` = var1;

END $$
DELIMITER ;

CALL contact_pharmacy(8);
